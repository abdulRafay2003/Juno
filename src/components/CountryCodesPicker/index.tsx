...
export const CountryPicker = ({
  ...
  const resultCountries = React.useMemo(() => {
    ...
    return dropDownData?.mobile_country_code.filter(country => {
      if (
        country?.key.includes(searchValue) ||
        country?.value.toLowerCase().includes(lowerSearchValue) ||
        removeDiacritics(country?.value.toLowerCase()).includes(
          lowerSearchValue,
        )
      ) {
        return country;
  ...
  const renderItem = ({item, index}: {item: CountryItem; index: number}) => {
    ...
    let checkName = itemName?.length
      ? itemName.substring(0, itemName.indexOf(':')).trim()
      : item?.value.substring(0, item?.value.indexOf(':')).trim();
    return (
      <ItemTemplate
        key={index}
        item={item}
...
//   const renderItem = ({item, index}: {item: CountryItem; index: number}) => {
//     let itemName = item?.value;
//     let checkName = itemName?.length ? itemName : item?.value;
//     console.log('itemName', itemName, 'checkName', checkName);
//     return (
//       <ItemTemplate
//         key={index}
//         item={item}
