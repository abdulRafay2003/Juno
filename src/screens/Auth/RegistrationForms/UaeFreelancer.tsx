...
import {
  step1FreelanceUAESchema,
  step2FreelanceUAESchema,
  step4SchemaUAEFreelancer,
...
import UaeFreelancerSteps from '@/components/RegistrationSteps/FormSteps/UaeFreelancerSteps';
...
const UaeFreelancer = () => {
  ...
  const getStepDataQuery = useQuery({
    queryKey: ['onboardingUAEFreelance', selectedSteps],
    ...
    switch (selectedSteps) {
      case 1:
        ...
        if (stepData?.signatory?.email)
          setValue('signatoryEmail', stepData?.signatory?.email?.toLowerCase());
        if (stepData?.signatory?.national_id_number)
          setValue('emiratesIdNum', stepData?.signatory?.national_id_number);
