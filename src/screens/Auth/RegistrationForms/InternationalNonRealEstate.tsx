...
import {
  internationNonRealEstateStep1Schema,
  internationNonRealEstateStep2Schema,
  internationNonRealEstateStep3Schema,
  internationNonRealEstateStep4Schema,
  step1Schema,
  step2Schema,
...
import InternationalNonRealEstateSteps from '@/components/RegistrationSteps/FormSteps/InternationalNonRealEstateSteps';
...
const InternationalNonRealEstate = () => {
  ...
  const {
    ...
    context: {
      freelancer: false,
      checkType: true,
      headerTitle: 'International Non Real Estate Company',
  ...
  const getStepDataQuery = useQuery({
    queryKey: ['onboardingInternationalNonRealEstate', selectedSteps],
    ...
    switch (selectedSteps) {
      case 1:
        ...
        if (stepData.email) {
          setValue(
            'companyEmail',
            stepData.email?.toLowerCase() ?? stepData.email,
          );
        }
    ...
    try {
      ...
      if (selectedSteps === 4) {
        ...
        const stepData = {
          step: 'SUBMIT',
          data: {
            category: 'International Non Real Estate Company',
          },
        };
    ...
    switch (selectedSteps) {
      case 1:
        ...
        submitData.data.data = cleanData({
          ...
          city: formData.city,
          city_id: formData.city === '' ? null : CityId?.id,
  ...
  const renderSteps = () => {
    return (
      <InternationalNonRealEstateSteps
  ...
  return (
    <MainContainer
      isHeader
      heading="International Non-Real Estate Company"
      isFlatList
...
export default InternationalNonRealEstate;
