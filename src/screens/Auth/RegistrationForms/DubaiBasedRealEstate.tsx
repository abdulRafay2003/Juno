...
import {
  step1DubaiBasedSchema,
  step1Schema,
  step2DubaiBasedSchema,
  step2Schema,
  step3DubaiBasedSchema,
  step3Schema,
  step4Schema,
...
import DubaiBasedRealEstateSteps from '@/components/RegistrationSteps/FormSteps/DubaiBasedRealEstateSteps';
...
const DubaiBasedRealEstate = () => {
  ...
  const trn = watch('haveTrn');
  const tradeLicenseCategory = watch('tradeLicenseCategory');
  ...
  useEffect(() => {
    if (trn === 'Yes') {
      setValue('nonVatDisclosure', null);
    } else {
      setValue('vatCertificate', null);
    }
  }, [trn]);
  // useEffect(() => {
  //   if (tradeLicenseCategory == 'DED') {
  //   } else if (tradeLicenseCategory == 'Freezone') {
  //     setValue('companyReraNumber', '');
  //     setValue('brokenReraNumber', '');
  //     setValue('companyReraRegistrationExpiry', undefined);
  //     setValue('reraRegistrationExpiryDate', undefined);
  //   } else {
  //     const options = dropDownData?.trade_licence_category.find(itemx => {
  //       return itemx?.id === tradeLicenseCategory;
  //     });
  //     if (options?.key?.toLowerCase() === 'freezone') {
  //       setValue('companyReraNumber', '');
  //       setValue('brokenReraNumber', '');
  //       setValue('companyReraRegistrationExpiry', undefined);
  //       setValue('reraRegistrationExpiryDate', undefined);
  ...
  useEffect(() => {
    ...
    const city = dropDownData.uae_city.find(item => item?.is_default === true);
    if (city) {
    ...
    if (selectedSteps === 3) {
      ...
      setValue('validTradeLicense', null);
      setValue('reraCertificate', null);
      setValue('brokerCard', null);
  ...
  const getStepDataQuery = useQuery({
    queryKey: ['onboardingDubai', selectedSteps],
  ...
  useEffect(() => {
    if (dropDownData?.country) {
      ...
      if (countryOption) {
        ...
        // setValue('bankCountry', countryOption.value);
      }
    }
    ...
    switch (selectedSteps) {
      case 1:
        ...
        if (stepData.city_id) {
          ...
          setValue('city', cityOptions.value);
        }
        ...
        if (stepData.rera_number)
          setValue('companyReraNumber', stepData.rera_number);
        if (stepData.rera_registration_expiry_date)
          setValue(
            'companyReraRegistrationExpiry',
            new Date(stepData.rera_registration_expiry_date),
          );
        ...
        if (stepData.ownership_id) {
          ...
          if (ownershipOption) setValue('ownership', ownershipOption.id);
        }
      ...
      case 2:
        ...
        if (stepData.broker_rera_number)
          setValue('brokenReraNumber', stepData.broker_rera_number);
        if (stepData.broker_rera_registration_expiry_date)
          setValue(
            'reraRegistrationExpiryDate',
            new Date(stepData.broker_rera_registration_expiry_date),
          );
      ...
      case 3:
        ...
        setBankAvailability(stepData.is_bank_detail_available);
        if (stepData.country_id) {
  ...
  const submitStepMutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      console.error('❌ Step post failed:', err?.response?.data);
      showToast('error', 'Error', err?.response?.data?.message as string);
  ...
  const submitStepData = async (formData: any) => {
    ...
    switch (selectedSteps) {
      case 1:
        ...
        submitData.data.data = cleanData({
          ...
          ...(findDropdownOptionById(
            ...
            rera_number: formData.companyReraNumber,
            rera_registration_expiry_date:
              formData.companyReraRegistrationExpiry,
          }),
      ...
      case 2:
        ...
        submitData.data.data = cleanData({
          ...
          ...(findDropdownOptionById(
            ...
            broker_rera_number: formData.brokenReraNumber,
            broker_rera_registration_expiry_date:
              formData.reraRegistrationExpiryDate,
          }),
        });
        break;
      ...
      case 3:
        ...
        submitData.data.data = bankAvailability
          ? cleanData({
    ...
    <MainContainer
      ...
      <ScrollView
        ...
        <View style={styles.containerStyle}>
          {selectedSteps == 3 && (
            <View>
              <CustomText.LargeBoldText customStyle={styles.bankDetailsLabel}>
...
export default DubaiBasedRealEstate;
